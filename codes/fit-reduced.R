...
for(rep in 1:1000){
	...
	reduced <- getRemoved()
	if(!has.HCE){
		HCE <- paste0("a1_01_", 1:14)
		reduced <- c(HCE, reduced)
	}
	...
	if(has.HCE){
		save(metrics, file = paste0("rda-r/reduced-HCE-rep", rep, ".rda"))
	}else{
		save(metrics, file = paste0("rda-r/reduced-noHCE-rep", rep, ".rda"))
	}
}